...
int OnboardCommands::OnboardCommand(const std::vector<std::string>& args) {
  ...
  if (!skip_daemon) {
    if (install_daemon ||
      ...
      if (SetupDaemon() != 0) {
        SPDLOG_WARN("Daemon setup failed, continuing");
      }
    }
  }
  ...
  if (SetupSkills() != 0) {
    SPDLOG_WARN("Skills setup had issues, but continuing");
  }

  // Step 5: Verification
  PrintStep(5, 5, "Verification");
  if (VerifySetup() != 0) {
    SPDLOG_WARN("Some verification checks failed");
  }
...
int OnboardCommands::QuickSetupCommand(
  ...
  if (SetupSkills() != 0) {
    SPDLOG_WARN("Skills setup had issues during quick setup, but continuing");
  }
...
int OnboardCommands::SetupSkills() {
  ...
  try {
    ...
    SPDLOG_WARN("Failed to create skills directory: {}", e.what());
    return 1;
  }
  ...
  for (const auto& skill : GetBuiltinSkills()) {
    ...
    try {
      ...
      f << skill.content;
      if (!f) {
        SPDLOG_WARN("Failed to install skill '{}': write error", skill.name);
        ++failed;
        continue;
      }
      ...
      SPDLOG_WARN("Failed to install skill '{}': {}", skill.name, e.what());
      ++failed;
    }
  }
...
bool OnboardCommands::CreateWorkspaceDirectory() {
  ...
  try {
    ...
    SPDLOG_ERROR("Failed to create workspace: {}", e.what());
    return false;
  }
}
...
bool OnboardCommands::CreateConfigFile(const std::string& model, int port,
  ...
  try {
    ...
    SPDLOG_ERROR("Failed to create config: {}", e.what());
    return false;
  }
}
...
bool OnboardCommands::CreateWorkspaceFile(const std::string& filename,
  ...
  try {
    ...
    SPDLOG_ERROR("Failed to create {}: {}", filename, e.what());
    return false;
  }
}
