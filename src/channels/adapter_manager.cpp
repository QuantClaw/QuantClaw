...
bool ChannelAdapterManager::launch_adapter(AdapterProcess& adapter,
  ...
  if (pid == platform::kInvalidPid) {
    SPDLOG_ERROR("Failed to launch adapter '{}'", adapter.name);
    return false;
  }

  adapter.pid = pid;
  adapter.running = true;
  SPDLOG_INFO("Launched adapter '{}' (PID: {}, script: {})", adapter.name, pid,
              adapter.script_path);
  return true;
}
...
void ChannelAdapterManager::kill_adapter(AdapterProcess& adapter) {
  ...
  SPDLOG_INFO("Stopping adapter '{}' (PID: {})", adapter.name, adapter.pid);
  ...
  if (exit_code >= 0) {
    ...
    SPDLOG_INFO("Adapter '{}' stopped", adapter.name);
    return;
  }
  ...
  adapter.running = false;
  adapter.pid = platform::kInvalidPid;
  SPDLOG_WARN("Adapter '{}' force-killed", adapter.name);
}
...
void ChannelAdapterManager::Start() {
  ...
  for (const auto& [name, config] : channels_) {
    if (!config.enabled) {
      SPDLOG_DEBUG("Channel '{}' disabled, skipping", name);
      continue;
    }

    std::string script = find_adapter_script(name);
    if (script.empty()) {
      SPDLOG_WARN("No adapter script found for channel '{}', skipping", name);
      continue;
    }
  ...
  if (!adapters_.empty()) {
    ...
    SPDLOG_INFO("ChannelAdapterManager started with {} adapter(s)",
                adapters_.size());
  } else {
    SPDLOG_INFO("No channel adapters to start");
  }
}
  ...
  while (running_) {
    ...
    for (auto& adapter : adapters_) {
      ...
      if (!platform::is_process_alive(adapter.pid)) {
        SPDLOG_WARN("Adapter '{}' exited unexpectedly", adapter.name);
        adapter.running = false;
        adapter.pid = platform::kInvalidPid;

        // Auto-restart if manager is still running
        if (running_) {
          SPDLOG_INFO("Restarting adapter '{}'...", adapter.name);
