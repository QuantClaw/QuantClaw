...
AgentLoop::AgentLoop(std::shared_ptr<MemoryManager> memory_manager,
  ...
  SPDLOG_INFO("AgentLoop initialized with model: {}, max_iterations: {}",
  ...
  if (failover_resolver_) {
    ...
    if (resolved) {
      ...
      if (resolved->is_fallback) {
        SPDLOG_INFO("Using fallback model: {}/{}", resolved->provider_id,
                    resolved->model);
      }
      return resolved->provider;
    }
    SPDLOG_ERROR("FailoverResolver exhausted all models/profiles for '{}'",
                 agent_config_.model);
...
void AgentLoop::SetModel(const std::string& model_ref) {
  ...
  SPDLOG_INFO("Model set to: {}", model_ref);
}
...
std::vector<Message> AgentLoop::ProcessMessage(
  ...
  SPDLOG_INFO("Processing message (non-streaming)");
    ...
    if (total > keep) {
      ...
      effective_history.insert(
          ...
          Message{"system", "[Context compaction: " + std::to_string(removed) +
                                ...
                                "and capabilities.]"});
      SPDLOG_INFO("Auto-compacted history: {} -> {} messages", total,
  ...
  while (iterations < max_iterations_ && !stop_requested_) {
    try {
      ...
      SPDLOG_DEBUG("Token usage: prompt={} completion={}",
                   response.usage.prompt_tokens,
                   response.usage.completion_tokens);
      ...
      if (!response.tool_calls.empty()) {
        SPDLOG_INFO("LLM requested {} tool calls", response.tool_calls.size());
      ...
      if (!response.content.empty()) {
        SPDLOG_INFO("LLM provided final response");
        Message final_msg;
      ...
      SPDLOG_ERROR("Unexpected LLM response format");
      break;
      ...
      if (pe.Kind() == ProviderErrorKind::kContextOverflow &&
        ...
        overflow_retries++;
        SPDLOG_WARN("Context overflow (attempt {}/{}), compacting and retrying",
                    overflow_retries, kOverflowCompactionMaxRetries);
      ...
      if (pe.Kind() == ProviderErrorKind::kContextOverflow) {
        SPDLOG_ERROR("Context overflow: all {} compaction retries exhausted",
      ...
      if (failover_resolver_ && !last_provider_id_.empty()) {
        ...
        SPDLOG_WARN("Provider error ({}), attempting failover: {}",
      ...
      SPDLOG_ERROR("Provider error with no failover available: {}", pe.what());
      ...
      SPDLOG_ERROR("Error in LLM processing: {}", e.what());
      if (iterations < max_iterations_ - 1) {
        std::this_thread::sleep_for(
            std::chrono::seconds(1 << std::min(iterations, 4)));
        iterations++;
...
std::vector<Message> AgentLoop::ProcessMessageStream(
  ...
  const std::string& effective_session_key =
      usage_session_key.empty() ? session_key_ : usage_session_key;
  SPDLOG_INFO("Processing message (streaming)");
  stop_requested_ = false;
    ...
    if (total > keep) {
      ...
      effective_history.insert(
          ...
          Message{"system", "[Context compaction: " + std::to_string(removed) +
                                ...
                                "Refer to the system prompt for your identity "
                                "and capabilities.]"});
      SPDLOG_INFO("Auto-compacted streaming history: {} -> {} messages", total,
                  static_cast<int>(effective_history.size()));
    }
  }
  ...
  if (estimated + agent_config_.max_tokens >
      ctx_window - kContextWindowMinTokens) {
    SPDLOG_WARN("Streaming context window guard triggered: {} + {} > {} - {}",
                estimated, agent_config_.max_tokens, ctx_window,
                kContextWindowMinTokens);
    int keep = std::min(agent_config_.compact_keep_recent,
                        static_cast<int>(context.size()));
  ...
  while (iterations < max_iterations_ && !stop_requested_) {
    try {
      ...
      SPDLOG_DEBUG("Token usage (stream): prompt={} completion={}",
      ...
      if (pe.Kind() == ProviderErrorKind::kContextOverflow &&
        ...
        SPDLOG_WARN("Streaming context overflow (attempt {}/{}), compacting",
                    overflow_retries_stream, kOverflowCompactionMaxRetries);
        int keep = std::max(2, static_cast<int>(request.messages.size()) / 2);
        std::vector<Message> compacted;
        if (!system_prompt.empty()) {
      ...
      if (pe.Kind() == ProviderErrorKind::kContextOverflow) {
        SPDLOG_ERROR("Streaming context overflow: retries exhausted");
        if (callback) {
      ...
      if (failover_resolver_ && !last_provider_id_.empty()) {
        ...
        SPDLOG_WARN("Streaming provider error ({}), attempting failover: {}",
                    ProviderErrorKindToString(pe.Kind()), pe.what());
      ...
      SPDLOG_ERROR("Error in streaming: {}", pe.what());
      if (callback) {
        callback({events::kMessageEnd, {{"error", pe.what()}}});
      }
      return new_messages;
      ...
      SPDLOG_ERROR("Error in streaming: {}", e.what());
      if (callback) {
...
void AgentLoop::Stop() {
  ...
  SPDLOG_INFO("Agent stop requested");
}
...
AgentLoop::handle_tool_calls(const std::vector<nlohmann::json>& tool_calls) {
  ...
  for (const auto& tool_call : tool_calls) {
    try {
      ...
      SPDLOG_INFO("Executing tool: {} with arguments: {}", tool_name,
                  arguments.dump());
      std::string result = tool_registry_->ExecuteTool(tool_name, arguments);
      results.push_back(result);
      SPDLOG_INFO("Tool execution successful");

    } catch (const std::exception& e) {
      SPDLOG_ERROR("Tool execution failed: {}", e.what());
