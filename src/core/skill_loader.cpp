...
SkillLoader::LoadSkillsFromDirectory(const std::filesystem::path& skills_dir) {
  ...
  if (!std::filesystem::exists(skills_dir)) {
    SPDLOG_DEBUG("Skills directory does not exist: {}", skills_dir.string());
    return skills;
  }

  SPDLOG_INFO("Loading skills from: {}", skills_dir.string());
    ...
    if (entry.is_regular_file() && entry.path().filename() == "SKILL.md") {
      try {
        ...
        if (CheckSkillGating(skill)) {
          SPDLOG_DEBUG("Loaded skill: {}", skill.name);
          skills.push_back(std::move(skill));
        } else {
          SPDLOG_DEBUG("Skipped skill (gating failed): {}", skill.name);
        }
      } catch (const std::exception& e) {
        SPDLOG_ERROR("Failed to load skill from {}: {}", entry.path().string(),
                     e.what());
      }
    }
  }

  SPDLOG_INFO("Loaded {} skills", skills.size());
  return skills;
}
...
bool SkillLoader::CheckSkillGating(const SkillMetadata& skill) {
  ...
  if (!skill.os_restrict.empty()) {
    if (!check_os_restriction(skill.os_restrict)) {
      SPDLOG_DEBUG("Skill gating failed: OS not in allowed list");
      return false;
    }
  }
  ...
  for (const auto& binary : skill.required_bins) {
    if (!is_binary_available(binary)) {
      SPDLOG_DEBUG("Skill gating failed: binary '{}' not available", binary);
      return false;
    }
  }
  ...
  if (!skill.any_bins.empty()) {
    ...
    if (!found) {
      SPDLOG_DEBUG("Skill gating failed: none of anyBins available");
      return false;
    }
  }
  ...
  for (const auto& env_var : skill.required_envs) {
    if (!is_env_var_available(env_var)) {
      SPDLOG_DEBUG("Skill gating failed: env '{}' not available", env_var);
      return false;
    }
  }
  ...
  for (const auto& config_file : skill.config_files) {
    ...
    if (!std::filesystem::exists(expanded)) {
      SPDLOG_DEBUG("Skill gating failed: config '{}' not found", config_file);
      return false;
    }
  }
...
bool SkillLoader::InstallSkill(const SkillMetadata& skill) {
  if (skill.installs.empty()) {
    SPDLOG_INFO("Skill '{}' has no install instructions", skill.name);
    return true;
  }
  ...
  for (const auto& inst : skill.installs) {
    ...
    if (!eff_binary.empty() && is_binary_available(eff_binary)) {
      SPDLOG_DEBUG("Skill '{}': {} already installed", skill.name, eff_binary);
      continue;
    }
    ...
    if (eff_method == "node") {
      ...
      SPDLOG_WARN("Skill '{}': unknown install method '{}'", skill.name,
                  eff_method);
      continue;
    }

    SPDLOG_INFO("Installing skill '{}' via {}: {}", skill.name, eff_method,
                cmd);
    int ret = std::system(cmd.c_str());
    if (ret != 0) {
      SPDLOG_ERROR("Skill '{}' install failed (exit {})", skill.name, ret);
      all_ok = false;
    }
  }
  return all_ok;
...
SkillLoader::parse_skill_file(const std::filesystem::path& skill_file) const {
  ...
  if (std::regex_search(file_content, matches, frontmatter_regex)) {
    ...
    try {
      ...
      SPDLOG_WARN("Failed to parse skill frontmatter: {}", e.what());
    }
  }
...
SkillLoader::LoadSkills(const SkillsConfig& skills_config,
  ...
  for (const auto& dir : dirs) {
    ...
    for (auto& skill : skills) {
      ...
      if (it != skills_config.entries.end() && !it->second.enabled) {
        SPDLOG_DEBUG("Skill '{}' disabled via config", skill.name);
        continue;
      }
  ...
  SPDLOG_INFO("Loaded {} skills from {} directories", result.size(),
              dirs.size());
  return result;
}
