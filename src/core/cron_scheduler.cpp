...
void CronScheduler::Load(const std::string& filepath) {
  ...
  try {
    ...
    SPDLOG_INFO("Loaded {} cron jobs from {}", jobs_.size(), filepath);
  } catch (const std::exception& e) {
    SPDLOG_ERROR("Failed to load cron jobs: {}", e.what());
  }
}
...
std::string CronScheduler::AddJob(const std::string& name,
  ...
  SPDLOG_INFO("Added cron job '{}' ({}): {}", name, id, schedule);
  return id;
}
    ...
    for (auto* job : due_jobs) {
      if (handler_) {
        try {
          ...
          SPDLOG_ERROR("Cron job '{}' failed: {}", job->name, e.what());
        }
      }
