...
SessionCompaction::Compact(const std::vector<nlohmann::json>& messages,
  ...
  if (summary_fn) {
    try {
      ...
      SPDLOG_ERROR("Compaction summary failed: {}", e.what());
  ...
  SPDLOG_INFO("Compacted {} messages into summary + {} recent", to_summarize,
              keep);
  return result;
}
...
SessionCompaction::Truncate(const std::vector<nlohmann::json>& messages,
  ...
  SPDLOG_INFO("Truncated {} messages, keeping {} recent", total - keep, keep);
  return result;
}
