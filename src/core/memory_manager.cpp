...
MemoryManager::MemoryManager(const std::filesystem::path& workspace_path,
  ...
  SPDLOG_INFO("MemoryManager initialized with workspace: {}",
...
void MemoryManager::LoadWorkspaceFiles() {
  SPDLOG_INFO("Loading workspace files from: {}", workspace_path_.string());
  ...
  for (const auto& name :
    ...
    try {
      ...
      if (!content.empty()) {
        SPDLOG_DEBUG("Loaded {} ({} bytes)", name, content.size());
      }
    } catch (const std::exception& e) {
      SPDLOG_DEBUG("No {} found: {}", name, e.what());
    }
  }
    ...
    for (const auto& entry : std::filesystem::directory_iterator(memory_dir)) {
      if (entry.is_regular_file() && entry.path().extension() == ".md") {
        try {
          ...
          SPDLOG_WARN("Failed to load memory file {}: {}",
                      entry.path().filename().string(), e.what());
        }
      }
    }
    SPDLOG_INFO("Loaded {} daily memory files", loaded_count);
  }

  SPDLOG_INFO("Workspace files loaded successfully");
}
...
void MemoryManager::SaveDailyMemory(const std::string& content) {
  ...
  if (file.is_open()) {
    file << entry_content;
    SPDLOG_DEBUG("Saved memory entry to {}", memory_file.string());
  } else {
    SPDLOG_ERROR("Failed to save memory entry to {}", memory_file.string());
...
void MemoryManager::StartFileWatcher() {
  ...
  SPDLOG_INFO("File watcher started for workspace: {}",
              workspace_path_.string());
}
...
void MemoryManager::StopFileWatcher() {
  ...
  SPDLOG_INFO("File watcher stopped");
}
...
void MemoryManager::SetAgentWorkspace(const std::string& agent_id) {
  ...
  std::filesystem::create_directories(workspace_path_);
  SPDLOG_INFO("Set agent workspace: {}", workspace_path_.string());
}
