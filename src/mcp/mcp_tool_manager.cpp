...
void MCPToolManager::DiscoverTools(const MCPConfig& config) {
  for (const auto& server_cfg : config.servers) {
    if (server_cfg.name.empty() || server_cfg.url.empty()) {
      SPDLOG_WARN("Skipping MCP server with empty name or URL");
      continue;
    }

    SPDLOG_INFO("Discovering tools from MCP server '{}' at {}", server_cfg.name,
                server_cfg.url);
    ...
    try {
      ...
      for (const auto& tool : tools) {
        ...
        SPDLOG_INFO("Discovered MCP tool: {} -> {}", qualified,
                    tool.description);
      }

      SPDLOG_INFO("Discovered {} tools from server '{}'", tools.size(),
                  server_cfg.name);
    } catch (const std::exception& e) {
      SPDLOG_ERROR("Failed to discover tools from '{}': {}", server_cfg.name,
                   e.what());
    }
  }

  SPDLOG_INFO("Total MCP tools discovered: {}", tool_to_server_.size());
}
...
void MCPToolManager::RegisterInto(ToolRegistry& registry) {
  ...
  SPDLOG_INFO("Registered {} MCP tools into ToolRegistry", tool_meta_.size());
}
...
std::string MCPToolManager::ExecuteTool(const std::string& qualified_name,
  ...
  SPDLOG_DEBUG("Executing MCP tool '{}' (server: '{}', original: '{}')",
