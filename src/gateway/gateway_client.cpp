...
bool GatewayClient::Connect(int timeout_ms) {
  ...
  if (!connected_) {
    SPDLOG_ERROR("Connection timeout to {}", url_);
    ws_.stop();
    return false;
  }
  ...
  if (!authenticated_) {
    SPDLOG_WARN("Connected but not yet authenticated");
  }

  SPDLOG_INFO("Connected to gateway at {}", url_);
  return true;
}
...
void GatewayClient::Disconnect() {
  if (connected_) {
    ...
    SPDLOG_INFO("Disconnected from gateway");
  }
}
...
void GatewayClient::on_message(const ix::WebSocketMessagePtr& msg) {
  switch (msg->type) {
    case ix::WebSocketMessageType::Open:
      connected_ = true;
      SPDLOG_DEBUG("WebSocket connection opened");
      break;
    ...
    case ix::WebSocketMessageType::Message: {
      try {
        ...
        SPDLOG_ERROR("Failed to parse message: {}", e.what());
      }
      break;
    }
