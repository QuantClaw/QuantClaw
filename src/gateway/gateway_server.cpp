...
void GatewayServer::Start() {
  if (running_) {
    SPDLOG_WARN("GatewayServer already running");
    return;
  }
  ...
  SPDLOG_INFO("GatewayServer started on port {}", port_);
}
...
void GatewayServer::Stop() {
  ...
  SPDLOG_INFO("GatewayServer stopped");
}
...
void GatewayServer::RegisterHandler(const std::string& method,
  ...
  SPDLOG_DEBUG("Registered RPC handler: {}", method);
}
...
void GatewayServer::on_connection(std::shared_ptr<ix::ConnectionState> state,
  ...
  switch (msg->type) {
    case ix::WebSocketMessageType::Open: {
      SPDLOG_INFO("Client connected: {}", conn_id);
    ...
    case ix::WebSocketMessageType::Close: {
      SPDLOG_INFO("Client disconnected: {}", conn_id);
      std::lock_guard<std::mutex> lock(connections_mutex_);
    ...
    case ix::WebSocketMessageType::Error: {
      SPDLOG_ERROR("WebSocket error for {}: {}", conn_id,
...
void GatewayServer::handle_message(const std::string& conn_id,
  ...
  try {
    ...
    switch (type) {
      ...
      default:
        SPDLOG_WARN("Unexpected frame type from client: {}", data);
        break;
    }
  } catch (const std::exception& e) {
    SPDLOG_ERROR("Failed to parse message from {}: {}", conn_id, e.what());
  }
}
...
void GatewayServer::handle_rpc_request(const std::string& conn_id,
  ...
  SPDLOG_INFO("RPC request from {}: {} (id={})", conn_id, request.method,
              request.id);
  ...
  if (rbac_checker_ && client->authenticated) {
    if (!rbac_checker_->IsAllowed(request.method, client->role,
                                  client->scopes)) {
      SPDLOG_WARN("RBAC denied: method={}, role={}, conn={}", request.method,
                  client->role, conn_id);
      auto resp = RpcResponse::failure(
          request.id,
  ...
  if (rate_limiter_) {
    if (!rate_limiter_->Allow(conn_id)) {
      ...
      SPDLOG_WARN("Rate limited: conn={}, retry_after={}s", conn_id, retry);
  ...
  try {
    ...
    SPDLOG_ERROR("RPC handler error for {}: {}", request.method, e.what());
...
void GatewayServer::SetAuth(const std::string& mode, const std::string& token) {
  ...
  SPDLOG_INFO("Gateway auth configured: mode={}", mode);
}
...
bool GatewayServer::handle_hello(const std::string& conn_id,
  ...
  // If auth mode is "token", validate the token
  {
    ...
    if (auth_mode_ == "token" && !expected_token_.empty()) {
      if (hello.auth_token != expected_token_) {
        SPDLOG_WARN("Auth failed for {}: bad token", conn_id);
        return false;
      }
    }
  }
  ...
  SPDLOG_INFO("Client {} authenticated: role={}, client={}, type={}", conn_id,
