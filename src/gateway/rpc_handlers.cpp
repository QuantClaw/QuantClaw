...
void register_rpc_handlers(
    ...
    // before forwarding to the LLM.
    {
      ...
      cmd_handlers.compact_session = [session_manager](const std::string& key) {
        ...
        if (history.size() > 20) {
          ...
          SPDLOG_INFO("Compacted session {}: kept {} of {} messages", key, keep,
                      history.size());
        }
      };
  ...
  if (skill_loader) {
    server.RegisterHandler(
        ...
        [](const nlohmann::json& params,
          ...
          SPDLOG_INFO("skills.update: skillKey={} enabled={}", skill_key,
                      enabled);
  ...
  SPDLOG_INFO("Registered {} RPC handlers", handler_count);
}
