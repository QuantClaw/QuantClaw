...
void CommandQueue::Start() {
  ...
  SPDLOG_INFO("CommandQueue started (maxConcurrent={})",
...
void CommandQueue::Stop() {
  ...
  SPDLOG_INFO("CommandQueue stopped");
}
...
std::string CommandQueue::Submit(const std::string& session_key,
...

  {
    ...
    if (cmd.mode == QueueMode::kInterrupt && lane.HasActive()) {
      ...
      if (aborted_id) {
        SPDLOG_INFO("Interrupted active command {} for session {}", *aborted_id,
                    session_key);
      }
    }
    ...
    if (!dropped.empty()) {
      for (const auto& did : dropped) {
        SPDLOG_WARN("Queue overflow: collapsed command {} for session {}", did,
                    session_key);
  ...
  cv_.notify_one();
  SPDLOG_DEBUG("Queued command {} for session {}", cmd_id, session_key);
  return cmd_id;
}
...
bool CommandQueue::AbortSession(const std::string& session_key) {
  ...
  if (aborted_id) {
    ...
    SPDLOG_INFO("Aborted active command for session {}", session_key);
    return true;
  }
  return false;
}
...
void CommandQueue::execute_command(QueuedCommand cmd) {
  SPDLOG_DEBUG("Executing command {} for session {}", cmd.id, cmd.session_key);
  ...
  try {
    ...
    SPDLOG_ERROR("Command {} failed: {}", cmd.id, e.what());
    if (!cmd.rpc_request_id.empty()) {
