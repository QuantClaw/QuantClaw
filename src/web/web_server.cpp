...
void WebServer::AddRawRoute(const std::string& path, const std::string& method,
  ...
  SPDLOG_DEBUG("Added raw route: {} {}", method, path);
}
...
void WebServer::SetMountPoint(const std::string& mount,
  ...
  SPDLOG_DEBUG("Added mount point: {} -> {}", mount, dir);
}

void WebServer::Start() {
  if (running_) {
    SPDLOG_WARN("WebServer already running");
    return;
  }

  running_ = true;
  server_thread_ = std::make_unique<std::thread>(&WebServer::server_loop, this);
  SPDLOG_INFO("WebServer started on port {}", port_);
}
...
void WebServer::server_loop() {
    ...
    if (info.method == "GET") {
      http_server_->Get(path, [&info, this](const httplib::Request& req,
        ...
        try {
          ...
          SPDLOG_ERROR("Route handler failed: {}", e.what());
          res.status = 500;
  ...
  for (const auto& [mount, dir] : mount_points_) {
    if (http_server_->set_mount_point(mount, dir)) {
      SPDLOG_INFO("Mounted static files: {} -> {}", mount, dir);
    } else {
      SPDLOG_WARN("Failed to mount static files: {} -> {}", mount, dir);
    }
  }

  SPDLOG_INFO("HTTP server listening on 0.0.0.0:{}", port_);
