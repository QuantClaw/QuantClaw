...
AnthropicProvider::AnthropicProvider(const std::string& api_key,
  ...
  SPDLOG_INFO("AnthropicProvider initialized with base_url: {}", base_url_);
}
...
AnthropicProvider::ChatCompletion(const ChatCompletionRequest& request) {
  ...
  SPDLOG_DEBUG("Sending request to Anthropic API: {}", json_payload);

  std::string response = MakeApiRequest(json_payload);
  SPDLOG_DEBUG("Received response from Anthropic API: {}", response);
...
AnthropicProvider::MakeApiRequest(const std::string& json_payload) const {
  ...
  if (http_code >= 400) {
    ...
    if (retry_capture.retry_after_seconds > 0) {
      SPDLOG_WARN("Anthropic API HTTP {}: rate limited, retry-after={}s",
                  http_code, retry_capture.retry_after_seconds);
    } else {
      SPDLOG_ERROR("Anthropic API HTTP {}: {}", http_code,
...
void AnthropicProvider::ChatCompletionStream(
  ...
  std::string json_payload = payload.dump();
  SPDLOG_DEBUG("Sending streaming request to Anthropic API");
  ...
  if (http_code >= 400) {
    ...
    if (retry_capture.retry_after_seconds > 0) {
      SPDLOG_WARN("Anthropic streaming HTTP {}: rate limited, retry-after={}s",
                  http_code, retry_capture.retry_after_seconds);
    } else {
      SPDLOG_ERROR("Anthropic streaming HTTP {}", http_code);
    }
    ProviderError err(error_kind, static_cast<int>(http_code),
