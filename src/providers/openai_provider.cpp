...
OpenAIProvider::OpenAIProvider(const std::string& api_key,
  ...
  SPDLOG_INFO("OpenAIProvider initialized with base_url: {}", base_url_);
}
...
OpenAIProvider::ChatCompletion(const ChatCompletionRequest& request) {
  ...
  SPDLOG_DEBUG("Sending request to OpenAI API: {}", json_payload);

  std::string response = MakeApiRequest(json_payload);
  SPDLOG_DEBUG("Received response from OpenAI API: {}", response);
...
OpenAIProvider::MakeApiRequest(const std::string& json_payload) const {
  ...
  if (http_code >= 400) {
    ...
    if (retry_capture.retry_after_seconds > 0) {
      SPDLOG_WARN("OpenAI API HTTP {}: rate limited, retry-after={}s",
                  http_code, retry_capture.retry_after_seconds);
    } else {
      SPDLOG_ERROR("OpenAI API HTTP {}: {}", http_code,
...
void OpenAIProvider::ChatCompletionStream(
  ...
  std::string json_payload = payload.dump();
  SPDLOG_DEBUG("Sending streaming request to OpenAI API");
  ...
  if (http_code >= 400) {
    ...
    if (retry_capture.retry_after_seconds > 0) {
      SPDLOG_WARN("OpenAI streaming HTTP {}: rate limited, retry-after={}s",
                  http_code, retry_capture.retry_after_seconds);
    } else {
      SPDLOG_ERROR("OpenAI streaming HTTP {}", http_code);
    }
    ProviderError err(error_kind, static_cast<int>(http_code),
