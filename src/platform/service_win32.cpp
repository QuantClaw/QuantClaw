...
int ServiceManager::install(int port) {
  ...
  if (!out) {
    SPDLOG_ERROR("Cannot write service config: {}", svc);
    return 1;
  }
  ...
  SPDLOG_INFO("Service config written to {}", svc);
  return 0;
}
...
int ServiceManager::uninstall() {
  ...
  SPDLOG_INFO("Service config removed");
  return 0;
}
...
int ServiceManager::start() {
  ...
  if (pid == kInvalidPid) {
    SPDLOG_ERROR("Failed to start gateway");
    return 1;
  }
  write_pid(static_cast<int>(pid));
  SPDLOG_INFO("Gateway started (PID: {})", pid);
  return 0;
}
...
int ServiceManager::stop() {
  int pid = get_pid();
  if (pid <= 0) {
    SPDLOG_INFO("Gateway not running");
    return 0;
  }
...
int ServiceManager::status() {
  ...
  if (pid <= 0 || !is_process_alive(static_cast<ProcessId>(pid))) {
    SPDLOG_INFO("Gateway is not running");
    return 1;
  }
  SPDLOG_INFO("Gateway is running (PID: {})", pid);
  return 0;
}
