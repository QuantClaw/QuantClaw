...
create_logger(const std::string& log_level = "info",
  ...
  console_sink->set_pattern("[%Y-%m-%d %H:%M:%S.%e %P %t] [%^%l%$] [%s:%#] %v");
  ...
  if (!log_dir.empty()) {
    try {
      ...
      file_sink->set_pattern("[%Y-%m-%d %H:%M:%S.%e %P %t] [%l] [%s:%#] %v");
...
int main(int argc, char* argv[]) {
  ...
  auto gateway_cmds = std::make_shared<quantclaw::cli::GatewayCommands>();
  auto agent_cmds = std::make_shared<quantclaw::cli::AgentCommands>();
  auto session_cmds = std::make_shared<quantclaw::cli::SessionCommands>();
  auto onboard_cmds = std::make_shared<quantclaw::cli::OnboardCommands>();
  ...
  cli.AddCommand(
      {"health",
       ...
       {},
       [gateway_url, auth_token](int argc, char** argv) -> int {
  ...
  cli.AddCommand(
      {"channels",
       ...
       [gateway_url, auth_token](int argc, char** argv) -> int {
         ...
         auto make_client = [&gateway_url, &auth_token]()
         ...
         if (sub == "remove") {
           ...
           try {
             ...
             try {
               ...
               SPDLOG_DEBUG("plugins.installs.{} not found: {}", plugin_id,
                            ue.what());
             }
             try {
               quantclaw::QuantClawConfig::UnsetValue(
               ...
               SPDLOG_DEBUG("plugins.entries.{} not found: {}", plugin_id,
                            ue.what());
             }
