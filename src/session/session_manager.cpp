...
SessionHandle SessionManager::GetOrCreate(const std::string& session_key,
  ...
  SPDLOG_INFO("Created new session: {} -> {}", normalized, sid);
...
void SessionManager::AppendMessage(const std::string& session_key,
  ...
  auto it = store_.find(normalized);
  if (it == store_.end()) {
    SPDLOG_ERROR("Session not found: {}", session_key);
    return;
  }
  ...
  if (!file.is_open()) {
    SPDLOG_ERROR("Failed to open transcript: {}", path.string());
    return;
  }
...
SessionManager::GetHistory(const std::string& session_key,
  ...
  while (std::getline(file, line)) {
    ...
    try {
      ...
      SPDLOG_WARN("Failed to parse JSONL line: {}", e.what());
    }
  }
...
void SessionManager::DeleteSession(const std::string& session_key) {
  ...
  auto it = store_.find(normalized);
  if (it == store_.end()) {
    SPDLOG_WARN("Cannot delete non-existent session: {}", normalized);
    return;
  }
  ...
  SPDLOG_INFO("Deleted session: {}", normalized);
}
...
void SessionManager::ResetSession(const std::string& session_key) {
  ...
  auto it = store_.find(normalized);
  if (it == store_.end()) {
    SPDLOG_WARN("Cannot reset non-existent session: {}", session_key);
    return;
  }
  ...
  if (std::filesystem::exists(old_path)) {
    ...
    SPDLOG_INFO("Archived transcript: {}", archive_path.string());
  }
  ...
  SPDLOG_INFO("Reset session: {} -> {}", normalized, new_sid);
}
...
void SessionManager::LoadStore() {
  ...
  try {
    ...
    SPDLOG_INFO("Loaded {} sessions from store", store_.size());
  } catch (const std::exception& e) {
    SPDLOG_WARN("Failed to load sessions.json: {}", e.what());
  }
}
