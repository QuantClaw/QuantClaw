...
class MockOpenAIProvider : public quantclaw::OpenAIProvider {
 public:
  MockOpenAIProvider()
      : OpenAIProvider("test-key", "https://api.openai.com/v1", 30) {}
...
class OpenAIProviderTest : public ::testing::Test {
 protected:
  void SetUp() override {
    provider_ = std::make_unique<MockOpenAIProvider>();
  }
...
TEST_F(OpenAIProviderTest, ConstructionWithEmptyBaseUrl) {
  ...
  EXPECT_NO_THROW({ quantclaw::OpenAIProvider provider("key", "", 10); });
}
