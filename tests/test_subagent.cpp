...
TEST(SubagentConfigTest, FromJson) {
  ...
  auto c = SubagentConfig::from_json(j);
...
TEST(SubagentConfigTest, Defaults) {
  auto c = SubagentConfig::from_json(nlohmann::json::object());
...
TEST_F(SubagentManagerTest, CompleteRun) {
  ...
  auto* run = mgr_->get_run(result.run_id);
  ASSERT_NE(run, nullptr);
  EXPECT_EQ(run->state, SubagentRun::kRunning);

  mgr_->complete_run(result.run_id, "Done!");
  run = mgr_->get_run(result.run_id);
  ASSERT_NE(run, nullptr);
...
TEST_F(SubagentManagerTest, FailRun) {
  ...
  mgr_->fail_run(result.run_id, "Something broke");
  auto* run = mgr_->get_run(result.run_id);
  ASSERT_NE(run, nullptr);
...
TEST_F(SubagentManagerTest, CancelRun) {
  ...
  EXPECT_TRUE(mgr_->cancel_run(result.run_id));
  auto* run = mgr_->get_run(result.run_id);
  EXPECT_EQ(run->state, SubagentRun::kCancelled);

  // Can't cancel again
  EXPECT_FALSE(mgr_->cancel_run(result.run_id));
}
...
TEST_F(SubagentManagerTest, ActiveChildren) {
  ...
  auto children = mgr_->active_children("parent-a");
  EXPECT_EQ(children.size(), 2);

  // Complete one
  mgr_->complete_run(r1.run_id);
  children = mgr_->active_children("parent-a");
...
TEST_F(SubagentManagerTest, GetNonexistentRun) {
  auto* run = mgr_->get_run("nonexistent");
