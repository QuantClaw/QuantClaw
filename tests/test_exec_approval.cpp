...
TEST(AskModeTest, ParseStrings) {
  EXPECT_EQ(ask_mode_from_string("off"), AskMode::kOff);
  EXPECT_EQ(ask_mode_from_string("always"), AskMode::kAlways);
  EXPECT_EQ(ask_mode_from_string("on-miss"), AskMode::kOnMiss);
  EXPECT_EQ(ask_mode_from_string(""), AskMode::kOnMiss);  // default
}

TEST(AskModeTest, ToString) {
  EXPECT_EQ(ask_mode_to_string(AskMode::kOff), "off");
  EXPECT_EQ(ask_mode_to_string(AskMode::kAlways), "always");
  EXPECT_EQ(ask_mode_to_string(AskMode::kOnMiss), "on-miss");
}
...
TEST(ExecApprovalConfigTest, FromJson) {
  ...
  auto c = ExecApprovalConfig::from_json(j);
...
TEST(ExecApprovalConfigTest, Defaults) {
  auto c = ExecApprovalConfig::from_json(nlohmann::json::object());
...
TEST(ExecApprovalManagerTest, AskOffAutoApproves) {
  ...
  auto decision = mgr->request_approval("rm -rf /");
...
TEST(ExecApprovalManagerTest, OnMissAllowlistApproves) {
  ...
  EXPECT_EQ(mgr->request_approval("git status"), ApprovalDecision::kApproved);
  EXPECT_EQ(mgr->request_approval("ls"), ApprovalDecision::kApproved);
}
...
TEST(ExecApprovalManagerTest, AlwaysRequiresApproval) {
  ...
  auto decision = mgr->request_approval("ls");
...
TEST(ExecApprovalManagerTest, HandlerApproves) {
  ...
  mgr->set_approval_handler([](const ApprovalRequest&) {
...
TEST(ExecApprovalManagerTest, HandlerDenies) {
  ...
  mgr->set_approval_handler([](const ApprovalRequest& req) {
  ...
  EXPECT_EQ(mgr->request_approval("rm -rf /"), ApprovalDecision::kDenied);
  EXPECT_EQ(mgr->request_approval("ls"), ApprovalDecision::kApproved);
}
...
TEST(ExecApprovalManagerTest, ResolvedHistory) {
  ...
  mgr->request_approval("cmd1");
  mgr->request_approval("cmd2");
  ...
  mgr->request_approval("cmd3");
  auto history = mgr->resolved_history();
  EXPECT_GE(history.size(), 1);
}

TEST(ExecApprovalManagerTest, PruneExpired) {
  auto mgr = std::make_unique<ExecApprovalManager>(make_logger("approval"));
  mgr->prune_expired();  // Should not crash on empty
  EXPECT_TRUE(mgr->pending_requests().empty());
}

TEST(ExecApprovalManagerTest, DecisionToString) {
  EXPECT_EQ(approval_decision_to_string(ApprovalDecision::kApproved), "approved");
  EXPECT_EQ(approval_decision_to_string(ApprovalDecision::kDenied), "denied");
  EXPECT_EQ(approval_decision_to_string(ApprovalDecision::kTimeout), "timeout");
  EXPECT_EQ(approval_decision_to_string(ApprovalDecision::kPending), "pending");
}
