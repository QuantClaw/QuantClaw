...
class E2ETest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    memory_manager_ =
        std::make_shared<quantclaw::MemoryManager>(workspace_dir_);
    skill_loader_ = std::make_shared<quantclaw::SkillLoader>();
    tool_registry_ = std::make_shared<quantclaw::ToolRegistry>();
    ...
    session_manager_ =
        std::make_shared<quantclaw::SessionManager>(sessions_dir_);

    prompt_builder_ = std::make_shared<quantclaw::PromptBuilder>(
        memory_manager_, skill_loader_, tool_registry_);

    // Create server
    server_ = std::make_unique<quantclaw::gateway::GatewayServer>(port_);
  ...
  make_client(const std::string& token = "") {
    ...
    auto client =
        std::make_unique<quantclaw::gateway::GatewayClient>(url, token);
    return client;
  }
...
TEST_F(E2EAuthTest, E2E_AuthRejectNoHello) {
  ...
  std::string url = "ws://127.0.0.1:" + std::to_string(port_);
  quantclaw::gateway::GatewayClient client(url, "");
...
TEST_F(E2EAuthTest, E2E_AuthRejectBadToken) {
  std::string url = "ws://127.0.0.1:" + std::to_string(port_);
  quantclaw::gateway::GatewayClient client(url, "wrong-token");
...
TEST_F(E2EAuthTest, E2E_AuthSuccessCorrectToken) {
  std::string url = "ws://127.0.0.1:" + std::to_string(port_);
  quantclaw::gateway::GatewayClient client(url, "e2e-secret-token");
