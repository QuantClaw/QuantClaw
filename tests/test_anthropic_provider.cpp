...
class MockAnthropicProvider : public quantclaw::AnthropicProvider {
 public:
  MockAnthropicProvider()
      : AnthropicProvider("test-key", "https://api.anthropic.com", 30) {}
...
class AnthropicProviderTest : public ::testing::Test {
 protected:
  void SetUp() override {
    provider_ = std::make_unique<MockAnthropicProvider>();
  }
...
TEST_F(AnthropicProviderTest, ConstructionWithEmptyBaseUrl) {
  EXPECT_NO_THROW({ quantclaw::AnthropicProvider provider("key", "", 10); });
}
