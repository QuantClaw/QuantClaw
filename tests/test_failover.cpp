...
class FailoverResolverTest : public ::testing::Test {
 protected:
  void SetUp() override {
    registry_ = std::make_unique<ProviderRegistry>();
    ...
    registry_->RegisterFactory("openai", [](const ProviderEntry& entry) {
    ...
    resolver_ = std::make_unique<FailoverResolver>(registry_.get());
  }
